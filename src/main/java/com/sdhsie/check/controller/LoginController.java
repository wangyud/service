package com.sdhsie.check.controller;


import com.sdhsie.check.jopo.ErrorResponseEntity;
import com.sdhsie.check.model.User;
import com.sdhsie.check.service.UserService;
import com.sdhsie.check.utils.URIUtils;
...
@RequestMapping(value = "login", produces = {APPLICATION_JSON_UTF8_VALUE})
@RestController
@Api(value = "/login", tags = "LoginApi", description = "用户验证接口")
public class LoginController {
    ...
    public ResponseEntity addUser(@ApiParam(value = "用户信息") @RequestBody User user) {
        ...
        return ResponseEntity.created(URIUtils.createURI("/login/{id}", user.getId())).body(user);
